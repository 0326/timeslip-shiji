...
- **自由模式开放结局（必做）**：每条线 ≥3 个非死亡具名结局 = 1 个史实结局（`#ending:canon`）+ ≥2 条 IF 支线（`#ending:if_xxx`），注册进 `inkStories/<series>.ts` 的 `endings`（title/kind/epigraph）。IF 线创作法、铁律与范例见 **`docs/free-mode-design.md`**；验收 `node scripts/verify-free.mjs`。
...
| 标签 | 说明 |
|------|------|
| `#ending:<id>` | **写在结局收束文本行的行尾**（`-> END` 前最后一段文本上）；史实结局固定 `#ending:canon`，IF 结局 `#ending:if_xxx`；title/kind/epigraph 注册在 `inkStories/<series>.ts` 的 `endings`（见 `docs/free-mode-design.md`） |
...
- **`#correct` 必须＝史实路径（铁律）**：正史模式（canon，默认）由引擎强制——玩家选中非 `#correct` 的存活分支即判「偏离正史」失败；全 `#correct` 路径必须无死亡走到史实结局。**严禁把 `#correct` 标给"美德/明君"反事实项**（悲剧人物线如纣/夫差/幽王，`#correct`=顺其史实沉沦的那一项）。多结局/反事实照见支仅在自由模式（free）可达。成就标签是 `#achieve:`（不是 `#achievement:`）。
...
- [ ] `node scripts/verify-canon.mjs` 全绿（全 `#correct` 路径无死亡通关且触发成就——正史模式可玩性）
- [ ] `npm run build` 通过
