...
function readAll(srcPath) {
	return readFileSync(join(ROOT, srcPath), "utf8");
}
...
// minigames id 集合：从 minigames/index.ts 的 registerMinigame({ id: "xxx" }) 提取
const mgSrc = readAll("src/react-app/minigames/index.ts");
const MINIGAMES = new Set([...mgSrc.matchAll(/id:\s*"([^"]+)"/g)].map((m) => m[1]));

// ── 遍历一个已编译的 story，收集所有分支的标签 ──
function walk(story) {
	const used = { bg: new Set(), show: new Set(), death: new Set(), achieve: new Set(), minigame: new Set() };
	let deadEnds = 0;
	let maxDepth = 0;
	const seen = new Set();
	function collectTags(tags) {
		for (const t of tags || []) {
			...
			else if (k === "show" && v) used.show.add(v.split(":")[0]);
			else if (k === "death" && v) used.death.add(v);
			else if (k === "achieve" && v) used.achieve.add(v);
			else if (k === "minigame" && v) used.minigame.add(v.split(":")[0]);
		}
	}
	function play(depth) {
...
for (const f of files) {
	...
	const missMg = [...res.used.minigame].filter((x) => !MINIGAMES.has(x));
	const problems = [];
	...
	if (missMg.length) problems.push(`未注册小游戏 #minigame: ${missMg.join(",")}`);
	if (problems.length) {
		console.log(`⚠️  ${f} — ${problems.join(" | ")}`);
		hadError = true;
	} else {
		console.log(
			`✅ ${f} — 编译+全分支OK (深度${res.maxDepth}; bg${res.used.bg.size}/show${res.used.show.size}/death${res.used.death.size}/achieve${res.used.achieve.size}/mg${res.used.minigame.size})`,
		);
	}
}
