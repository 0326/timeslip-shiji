...
export function AuthModal({ open, onClose, initialMode = "login" }: AuthModalProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (mode === "login") {
			...
			res = await register(username, password, nickname || username);
		}
		if (!res.ok) setError(res.message || "操作失败");
		setLoading(false);
	};
	...
	return (
		<Modal open={open} onClose={onClose} className="auth-modal">
			...
			<form className="auth-form" onSubmit={handleSubmit}>
				...
				{mode === "register" && (
					<div className="auth-field">
						<label className="auth-label">昵称 <span className="optional">选填</span></label>
						<input
							type="text"
							value={nickname}
				...
				<Button type="submit" variant="vermilion" block disabled={loading} size="lg">
					{loading ? (
