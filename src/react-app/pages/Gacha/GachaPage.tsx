  ...
  return (
    <div className="gacha-page">
      <div className="gacha-banner">
        <Badge tone="gold">
          {CURRENT_POOL.era === "chu_han" ? "楚汉争霸" : CURRENT_POOL.era} ·
          首发卡池
        </Badge>
        <h1 className="pool-title">{CURRENT_POOL.name}</h1>
        <div className="tagline">所有角色概率均等 · 无星级 · 永不付费</div>
        <div className="pity-line">
          <Shield size={14} />
          保底：连续 {CURRENT_POOL.rules.pityAt} 抽未出新角色，第{" "}
          {CURRENT_POOL.rules.pityAt + 1} 抽必出新 · 当前还需 {remainToPity}{" "}
          抽触发保底
        </div>
        <div className="gacha-actions">
          <Button
            ...
            cut
            onClick={() => doPull(1)}
            disabled={progress.gachaTickets < 1}
          >
            <Sparkles size={18} /> 单抽{" "}
            <span className="gacha-cost">（1 券）</span>
          </Button>
          <Button
            size="lg"
            cut
            variant="cyan"
            onClick={() => doPull(10)}
            disabled={progress.gachaTickets < 10}
          >
            <Sparkles size={18} /> 十连{" "}
            <span className="gacha-cost">（10 券）</span>
          </Button>
        ...
        <div className="gacha-wallet">
          拥有 🎟️ {progress.gachaTickets} 券 · 🔹 {progress.fragments} 碎片 ·
          已集 {owned.size}/{CURRENT_POOL.characters.length}
        </div>
      </div>
      ...
      <section className="pool-section">
        ...
        <div className="pool-grid">
          {CURRENT_POOL.characters.map((pc, i) => {
            const ch =
              characterMap.get(pc.characterId) ?? getCharacter(pc.characterId);
            if (!ch) return null;
            const has = owned.has(pc.characterId);
            return (
              <div
                key={pc.characterId}
                className={`game-card pool-card${has ? "" : " is-locked"}`}
                style={
                  {
                    "--accent": has ? ch.accent : undefined,
                    animationDelay: `calc(var(--stagger) * ${i})`,
                  } as CSSProperties
                }
              >
                <CharacterAvatar
                  ...
                  size={72}
                  locked={!has}
                  avatarUrl={has ? ch.avatarUrl : null}
                />
                <div className="nm">{has ? ch.name : "？？？"}</div>
                <div className="tt">
                  {has ? ch.title.split(" · ")[0] : "尚未解锁"}
                </div>
      ...
      <Modal open={shopOpen} onClose={() => setShopOpen(false)}>
        <div className="modal-pad">
          <h2 className="serif" style={{ fontSize: "var(--text-h2)" }}>
            碎片商店
          </h2>
          <p className="dim" style={{ fontSize: 13, marginTop: 8 }}>
            即使运气不佳，也能凭积累拿到想要的角色。{FRAGMENT_COSTS.character}{" "}
            碎片兑换任意指定角色。
          </p>
          ...
          <div className="shop-list">
            {CURRENT_POOL.characters
              ...
              .map((pc) => {
                const ch =
                  characterMap.get(pc.characterId) ??
                  getCharacter(pc.characterId)!;
                return (
...
function SummonOverlay({
  ...
  return (
    <div
      className={`summon-overlay tone-${tone} phase-${phase}`}
      onClick={phase === "reveal" ? onClose : skip}
    >
      ...
      {phase !== "reveal" ? (
        ...
        <>
          ...
          <div className="reveal-grid" onClick={(e) => e.stopPropagation()}>
            {results.map((r, i) => {
              const ch =
                characterMap.get(r.characterId) ?? getCharacter(r.characterId)!;
              return (
                <div
                  key={i}
