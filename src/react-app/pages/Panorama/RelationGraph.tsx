 ...
 * 视觉保留 2D 力导向、glyph 节点、owned 解锁状态。
 */
export function RelationGraph({ focus, ownedIds }: Props) {
	...
	useEffect(() => {
		...
		const neighborIds = nodes
			.filter((n) => !n.isMain)
			.map((n) => n.id)
			.filter((id) => CHARACTER_MAP[id]?.name === undefined || CHARACTER_MAP[id]?.name === id);
	...
	}, [mergedFocus, ownedIds, loading]);

	const usedTypes = Array.from(new Set(mergedFocus.relations.map((r) => r.type)));

	return (
		<div className="relation-graph">
			{loading && (
				<div style={{ textAlign: "center", padding: "40% 0", color: "var(--color-rice-dim)" }}>
					正在从史册中抽取关系网…
				</div>
			)}
			<svg ref={ref} className="relation-svg" style={{ display: loading ? "none" : "block" }} />
			{!loading && (
