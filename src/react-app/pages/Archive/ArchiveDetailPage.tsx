...
export default function ArchiveDetailPage() {
	...
	const isOwned = id ? ownedSet.has(id) : false;
		...
		>
			...
			<div className="ad-panel">
				...
				<div className="ad-badges">
					...
					{!isOwned && (
						<span className="ad-badge is-locked">未激活 · 不可穿越</span>
					)}
				</div>
			...
			<div className="ad-actions">
					...
					{isOwned &&
						playableStorylines.map((st) => (
							<button
								key={st.id}
								className="ad-act"
								onClick={() => navigate(`/play/${st.id}/${id}`)}
							>
					...
					{gameChar?.relatedStorylines?.map((sid) => {
						...
						if (playable && isOwned) return null; // 已在穿越按钮展示
						return (
							<button
								key={sid}
								className="ad-act"
